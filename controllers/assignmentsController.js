...
async function createAssignment(req, res) {
  ...
  try {
    ...
    const newAssignment = await pool.query(
      ...
      [subject, title, description, due_date, difficulty_level, driveData?.id || null, driveData?.webViewLink || null, attachmentName]
    );
  ...
  try {
    const result = await pool.query(
      `INSERT INTO private_notes (user_id, assignment_id, note_text, is_completed)
       ...
       DO UPDATE SET note_text = EXCLUDED.note_text, is_completed = EXCLUDED.is_completed, updated_at = CURRENT_TIMESTAMP
       RETURNING *`,
